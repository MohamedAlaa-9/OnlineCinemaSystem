...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const moviesContainer = document.querySelector('.row.row-cols-1.row-cols-md-2.row-cols-lg-4.g-4.mb-5');
        ...
        <div class="card movie-card shadow h-100">
          ...
          <div class="card-body">
            ...
            <div class="d-grid gap-2">
              <a href="movie-detail.html?name=${encodedTitle}" class="btn btn-primary">
      ...
      moviesContainer.innerHTML = `
        <div class="col-12 text-center text-white">
          ...
          <button class="btn btn-primary mt-3" onclick="window.location.reload()">
            <i class="fas fa-sync-alt me-2"></i>Show All Movies
          </button>
        </div>
      `;
    } else {
      moviesContainer.innerHTML = filteredMovies.map(movie => createMovieCard(movie)).join('');
    }
    ...
    try {
      ...
      if (moviesContainer) {
        if (!data.movies || data.movies.length === 0) {
          ...
          // If not searching, display current page movies
          if (!isSearchActive) {
          ...
          // Store movies for search functionality
          if (page === 1) {
  ...
  if (searchButton && searchInput) {
    const handleSearch = () => {
      ...
      if (!searchTerm) {
        // Reset to first page of all movies
  ...
  // Add to window for the check server status button
